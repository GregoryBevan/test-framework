...
public class ArtifactoryResolver {
	...
	private ArtifactoryResolver() {
		this.artifactoryResolver = Maven.configureResolver().withRemoteRepo(REPOSITORY_NAME, REPOSITORY_URL,
				REPOSITORY_LAYOUT);
	 ...
	 *
	 * @param dependency
	 *            Dependency with pattern 'group:artifact:version'
	 * @param transitive
	 *            Boolean for transitive resolution or not
	...
	public File[] getDependencyFiles(final String dependency, final boolean transitive) {
		...
		final MavenFormatStage mavenFormatStage = transitive ? resolve.withTransitivity()
